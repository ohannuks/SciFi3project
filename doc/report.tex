...
previously, in Uintah parallelization goes over patches. Every \emph{process} has an N amount of patches which they are in charge of, and those patches 
...
The parallel task system takes care of most data communication and working with data dependencies. For each function, we must define a task which takes in 
the \emph{function}, the function's \emph{dependencies} and the function's \emph{output}.
...
particle density $\rho$, and one poisson iteration calculates the next $\phi=\phi_{next}$ value.

In pseudocode, this would be:

\begin{algorithmic}
 \State $Task \gets $new task($poissonIteration$)
 \State $Task \gets$ $\phi_{previous}$
 \State $Task \gets$ $\rho_{current}$
 \State $Task$ computes $\phi_{next}$
